'''
Pulling BitShares assets as CSV Data
This file ref https://github.com/QuantSoftware/QuantSoftwareToolkit/blob/master/QSTK/qstktools/YahooDataPull.py 
and attempts to reproduce it's behavior against the bitshares network. 
Since all yahoo data is pulled relative to USD we will only use the USD base currency
for prices.  
In the future, we'll look into extending the data file to indicate the base unit
price used since bitshares supports it and this can be useful for various trading
scenarios.
...
def get_bts_data(data_path, ls_symbols):
    '''Read data from bitshares
    @data_path : string for where to place the output files
    @ls_symbols: list of symbols to read from bts
    '''
