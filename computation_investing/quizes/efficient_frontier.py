...
def process(ls_symbols, year):
    ...
    for s_sym in ls_bad_syms:
        ...
        print "Popping Symbol: %s" % (s_sym)
        ls_symbols.pop(i_index)

    # Start and End date of the charts
    dt_end = dt.datetime(int(year) , 1, 1)
    ...
    # Labels and Axis
    plt.legend(['%s Frontier' % (dt_start.year), '%s Frontier' % (dt_end.year),
        'Performance of %s Frontier in %s' %(dt_start.year, dt_end.year)], loc='lower right')
    plt.title('Efficient Frontier For S&P 100 ')
    plt.ylabel('Expected Return')
    plt.xlabel('StDev')
    plt.savefig('ef.pdf', format='pdf')
